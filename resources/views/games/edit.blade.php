    ...
    <div class="w-full max-w-xl bg-white rounded-xl shadow-2xl p-6 sm:p-10">
        ...
        <form action="{{ route('games.update', $game->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="platform" id="platform" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('platform', $game->platform) }}">
            </div>
            
            <div>
                <label for="genre" class="block text-sm font-medium text-gray-700">Gênero</label>
                <input type="text" name="genre" id="genre" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('genre', $game->genre) }}">
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    @php $currentStatus = old('status', $game->status); @endphp
                    <option value="Quero Jogar" @selected($currentStatus == 'Quero Jogar')>Quero Jogar</option>
                    <option value="Jogando" @selected($currentStatus == 'Jogando')>Jogando</option>
                    <option value="Finalizado" @selected($currentStatus == 'Finalizado')>Finalizado</option>
                    <option value="Pausado" @selected($currentStatus == 'Pausado')>Pausado</option>
                </select>
            </div>
            
            <div>
                <label for="rating" class="block text-sm font-medium text-gray-700">Nota (1-5)</label>
                <input type="number" name="rating" id="rating" min="1" max="5" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('rating', $game->rating) }}">
            </div>
            
            <div>
                <label for="cover_image_url" class="block text-sm font-medium text-gray-700">URL da Capa</label>
                <input type="url" name="cover_image_url" id="cover_image_url" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('cover_image_url', $game->cover_image_url) }}" placeholder="https://...">
            </div>
